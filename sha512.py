    ...
    for i in range(len(message)):
        ...
        for j in range(80):
            if j < 16:
                ...
                w[j] = addMod64(
                    addMod64(sigma1(w[j-2]), w[j-7]), 
                    addMod64(sigma0(w[j-15]), w[j-16])
                    )
        a = h[0]
        b = h[1]
        c = h[2]
        ...
        for t in range(80):
            T1 = addMod64(
                addMod64(
                addMod64(hh, sum1(e)), 
                addMod64(ch(e, f, g), k[t])
                ), w[t])
            T1 = (hh + sum1(e) + ch(e, f, g) + k[t] + w[t]) % int(math.pow(2, 64))
            T2 = (sum0(a) + maj(a, b, c)) % int(math.pow(2, 64))
            hh = g
            g = f
            f = e
            e = addMod64(d, T1)
